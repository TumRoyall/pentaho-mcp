...
| Bản phát hành client-neutral với cấu hình thủ công: `doctor.ps1` handshake initialize + tools/list (26 tool, từ chối `pentaho_*`, không kiểm prompt/resource) + dò PDI (thiếu PDI không fail); gỡ bỏ thủ công bằng cách xóa mục/bảng `dte-pentaho` khỏi cấu hình client | `scripts/build-release.mjs`, `test/packaging.test.js`, `packaging/doctor.ps1` | `docs/operations.md`, `docs/install.md` |
