...
| Source production hiện tại **không còn** phần cài đặt lifecycle BA (không module, không tool factory tương ứng); guard absence assert điều này, còn `registry.js` chỉ đăng ký 7 factory production | `test/legacy-removal.test.js`, `src/tools/registry.js` | `docs/architecture.md`, `docs/development.md` |
...
| Fact | Nguồn authoritative | Đích tài liệu |
|------|---------------------|----------------|
| Workspace root được phát hiện tự động bởi `src/workspace/resolve-root.js` (`resolveWorkspaceRoot`) theo thứ tự `~/.kettle/repositories.xml` → `PENTAHO_HOME/repositories.xml` → `process.cwd()` (file mode); trong `repositories.xml` repository mặc định (`is_default=Y`) thắng, nếu không thì repository đầu tiên có `base_directory` tồn tại. Biên luôn được enforce; canonical containment do `src/workspace/boundary.js` (`createWorkspaceBoundary`) sở hữu, dùng chung cho read/edit/validate/coverage | `src/workspace/resolve-root.js`, `src/workspace/boundary.js`, `src/tools/registry.js` | `README.md`, `docs/configuration.md`, `docs/architecture.md`, `docs/tools-reference.md` |
...
| Biến môi trường cấu hình server đọc: `PENTAHO_HOME` (bước 2 phát hiện root + vị trí PDI cho runtime), `PENTAHO_REPOSITORY_NAME` (override tên repository, mặc định lấy từ `repositories.xml`), `PENTAHO_ENABLE_EXECUTE` (**chỉ chuỗi `1`** mới bật), `KETTLE_KNOWLEDGE_DIR` (override knowledge nhúng). Biến môi trường đặt root cũ đã bị gỡ ở commit `7a6e490` — root nay chỉ được phát hiện tự động, không còn knob thủ công. `src/repository/registration.js` đọc thêm `KETTLE_HOME`/`USERPROFILE`/`HOME` như đường dự phòng khi dò `repositories.xml` cho nhóm tool repository | `src/server.js`, `src/knowledge/loader.js`, `src/workspace/resolve-root.js`, `src/repository/registration.js` | `docs/configuration.md`, `docs/README.md` |
| `PENTAHO_HOME` vừa là bước 2 của phát hiện workspace root (đọc `<PENTAHO_HOME>/repositories.xml`), vừa là thiết lập vị trí PDI duy nhất cho nhóm runtime tùy chọn; không đặt → runtime `UNAVAILABLE`, toàn bộ tool tĩnh vẫn chạy. Runtime dùng chung biên workspace root, log dưới `<workspace root>/.pentaho-mcp/runtime-logs`; execute luôn cần `confirmed: true` | `src/workspace/resolve-root.js`, `src/server.js`, `src/runtime/*`, `src/tools/runtime.tools.js` | `docs/configuration.md` |
...
| Bản phát hành client-neutral với cấu hình thủ công: `doctor.ps1` handshake initialize + tools/list (41 tool, từ chối `pentaho_*`, không kiểm prompt/resource) + dò PDI (thiếu PDI không fail); gỡ bỏ thủ công bằng cách xóa mục/bảng `dte-pentaho` khỏi cấu hình client | `scripts/build-release.mjs`, `test/packaging.test.js`, `packaging/doctor.ps1` | `docs/operations.md`, `docs/install.md` |
| Companion skill được đưa vào `files` của npm (mục `"skills"`) và copy vào ZIP release (`cpSync` đệ quy). **Discovery không tự động chỉ vì `skills/` trong ZIP**: Kiro dùng `.kiro/skills/developing-pentaho-jobs/` (workspace) hoặc `%USERPROFILE%/.kiro/skills/developing-pentaho-jobs/` (user); Claude Code dùng `.claude/skills/developing-pentaho-jobs/` (project) hoặc `%USERPROFILE%/.claude/skills/developing-pentaho-jobs/` (user); Codex dùng `.agents/skills/developing-pentaho-jobs/` (project) hoặc `%USERPROFILE%/.agents/skills/developing-pentaho-jobs/` (user) | `package.json`, `scripts/build-release.mjs`, `skills/developing-pentaho-jobs/SKILL.md` | `docs/install.md`, `docs/operations.md`, `README.md` |
